...
const densityClasses = [
  { max:    200,   col: '#fff18c', lab: '0 – 200'         }, // jaune clair
  { max:   1000,   col: '#6ad24c', lab: '201 – 1 000'     }, // vert vif
  { max:   6000,   col: '#14c8c4', lab: '1 001 – 6 000'   }, // turquoise
  { max:  15000,   col: '#1e6cff', lab: '6 001 – 15 000'  }, // bleu saturé
  { max: Infinity, col: '#6926ff', lab: '> 15 000'        }  // violet profond
];
...
fetch('recensement_secteur_2024.geojson')
  ...
  .then(data => {
    ...
    L.tileLayer(
      ...
      {
        attribution: 'Tiles © Esri — Sources: Esri, DeLorme, NAVTEQ',
        maxZoom: 16,
        tileSize: 256
      }
    ).addTo(map);
    ...
    const styleFeature = f => {
      if (currentMode === 'density') {
        ...
        return {
          fillColor: colorBy(dens, densityClasses),
          fillOpacity: .8, color: '#000', weight: 1
        };
      }
      const med = f.properties.medAge;
      return {
        fillColor: med == null ? pal.grey : colorBy(med, ageClasses),
        fillOpacity: .8, color: '#000', weight: 1
      };
    };
    ...
    if (L.Control?.Search)
      new L.Control.Search({
        layer: geo, propertyName: 'name_fr',
